...
extension UNTextInputNotificationResponse {
    static func response(trigger: UNNotificationTrigger?) -> UNTextInputNotificationResponse? {
        ...
        let textInputNotificationResponse = UNTextInputNotificationResponse(coder: NotificationResponseCoder(with: notificationRequest, notification: notification))!
    ...
    override func spec() {
        describe("RAnalyticsLaunchCollector") {
            ...
            it("should track the push notify event when a push notification is processed with report id") {
                ...
                expect(analyticsTrackerMock?.dictionary?[AnalyticsManager.Event.Name.pushNotification]?.tracked).toEventually(beTrue())
                expect((analyticsTrackerMock?.dictionary?[AnalyticsManager.Event.Name.pushNotification]?.parameters?[AnalyticsManager.Event.Parameter.pushTrackingIdentifier] as? String)?.hasPrefix("rid:1234abcd")).toEventually(beTrue())
            ...
            it("should track the push notify event when a push notification is processed with notification id") {
                ...
                launchCollector?.processPushNotificationPayload(userInfo: payload, userAction: nil, userText: nil)
                expect(analyticsTrackerMock?.dictionary?[AnalyticsManager.Event.Name.pushNotification]?.tracked).toEventually(beTrue())
                expect((analyticsTrackerMock?.dictionary?[AnalyticsManager.Event.Name.pushNotification]?.parameters?[AnalyticsManager.Event.Parameter.pushTrackingIdentifier] as? String)?.hasPrefix("nid:abcd1234")).toEventually(beTrue())
            ...
            it("should track the push notify event when a push notification is processed with string alert") {
                ...
                launchCollector?.processPushNotificationPayload(userInfo: payload, userAction: nil, userText: nil)
                expect(analyticsTrackerMock?.dictionary?[AnalyticsManager.Event.Name.pushNotification]?.tracked).toEventually(beTrue())
                expect((analyticsTrackerMock?.dictionary?[AnalyticsManager.Event.Name.pushNotification]?.parameters?[AnalyticsManager.Event.Parameter.pushTrackingIdentifier] as? String)?.hasPrefix("msg:")).toEventually(beTrue())
            ...
            it("should process UNNotificationResponse when the trigger is UNPushNotificationTrigger") {
                ...
                var processedTrigger: UNNotificationTrigger? = nil
                if case .success(let aTrigger) = result { processedTrigger = aTrigger }
                expect(processedTrigger).to(equal(trigger))
                expect(analyticsTrackerMock?.dictionary?[AnalyticsManager.Event.Name.pushNotification]?.tracked).to(beTrue())
                expect((analyticsTrackerMock?.dictionary?[AnalyticsManager.Event.Name.pushNotification]?.parameters?[AnalyticsManager.Event.Parameter.pushTrackingIdentifier] as? String)?.hasPrefix("rid:1234abcd")).to(beTrue())
            ...
            it("should not process UNNotificationResponse when the trigger is UNLocationNotificationTrigger") {
                ...
                expect(analyticsTrackerMock?.dictionary?[AnalyticsManager.Event.Name.pushNotification]?.tracked).to(beFalse())
                let result = launchCollector?.processPushNotificationResponse(textInputNotificationResponse)
                var error: RAnalyticsLaunchCollectorError? = nil
