...
- (NSUInteger)hash
{
    return self.sessionIdentifier.hash ^ self.deviceIdentifier.hash ^ self.currentVersion.hash ^ self.advertisingIdentifier.hash ^ self.lastKnownLocation.hash ^ self.sessionStartDate.hash ^ self.loggedIn ^ self.userIdentifier.hash ^ self.loginMethod ^ self.linkIdentifier.hash ^ self.origin ^ self.lastVisitedPage.hash ^ self.currentPage.hash ^ self.lastVersion.hash ^ self.lastVersionLaunches ^ self.initialLaunchDate.hash ^ self.installLaunchDate.hash ^ self.lastLaunchDate.hash ^ self.lastUpdateDate.hash;
}
...
- (BOOL)isEqual:(id)object
{
    ...
    else
    {
        ...
        return _RSDKAnalyticsObjects_equal(self.advertisingIdentifier, other.advertisingIdentifier) && _RSDKAnalyticsObjects_equal(self.sessionIdentifier, other.sessionIdentifier) && _RSDKAnalyticsObjects_equal(self.deviceIdentifier, other.deviceIdentifier) && _RSDKAnalyticsObjects_equal(self.currentVersion, other.currentVersion) && ([self.lastKnownLocation distanceFromLocation:other.lastKnownLocation] == 0) && _RSDKAnalyticsObjects_equal(self.sessionStartDate, other.sessionStartDate) && (self.loggedIn == other.loggedIn) && _RSDKAnalyticsObjects_equal(self.userIdentifier, other.userIdentifier) && (self.loginMethod == other.loginMethod) && _RSDKAnalyticsObjects_equal(self.linkIdentifier, other.linkIdentifier) && (self.origin == other.origin) && _RSDKAnalyticsObjects_equal(self.lastVisitedPage, other.lastVisitedPage) && _RSDKAnalyticsObjects_equal(self.currentPage, other.currentPage) && _RSDKAnalyticsObjects_equal(self.lastVersion, other.lastVersion) && (self.lastVersionLaunches == other.lastVersionLaunches) && _RSDKAnalyticsObjects_equal(self.initialLaunchDate, other.initialLaunchDate) && _RSDKAnalyticsObjects_equal(self.installLaunchDate, other.installLaunchDate) && _RSDKAnalyticsObjects_equal(self.lastLaunchDate, other.lastLaunchDate) && _RSDKAnalyticsObjects_equal(self.lastUpdateDate, other.lastUpdateDate);
    }
}
