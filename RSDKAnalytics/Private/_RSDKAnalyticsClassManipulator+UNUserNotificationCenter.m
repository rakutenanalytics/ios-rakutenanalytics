...
#ifdef RSDKA_BUILD_USER_NOTIFICATION_SUPPORT
@implementation _RSDKAnalyticsClassManipulator(UNNotificationCenter)
                      ...
                      withCompletionHandler:(void(^)())completionHandler
{
    ...
    if ([request.trigger isKindOfClass:UNPushNotificationTrigger.class])
    {
        ...
        RSDKAnalyticsDebugLog(@"Application did receive remote notification %@", payload);
        [_RSDKAnalyticsLaunchCollector.sharedInstance processPushNotificationPayload:payload
...
{
    RSDKAnalyticsDebugLog(@"User notification center delegate is being set to %@", delegate);
    if (!delegate) return;
    ...
    {
        [_RSDKAnalyticsClassManipulator addMethodWithSelector:selector
                                                      toClass:recipient
...
+ (void)load
{
    [_RSDKAnalyticsClassManipulator addMethodWithSelector:@selector(_r_autotrack_setUserNotificationCenterDelegate:)
                                                  toClass:UNUserNotificationCenter.class
                                                replacing:@selector(setDelegate:)
                                            onlyIfPresent:YES];
    RSDKAnalyticsDebugLog(@"Installed auto-tracking hooks for UNNotificationCenter.");
}
