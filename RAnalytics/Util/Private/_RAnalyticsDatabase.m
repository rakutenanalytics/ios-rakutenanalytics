...
static NSString *const RAnalyticsDatabaseName = @"RSDKAnalytics.db";

/*
 * Our global background queue (singleton, created in [_RAnalyticsDatabase load].
 */
...
static sqlite3 *prepareTable(NSString *table)
{
    ...
    if (![tables containsObject:table])
    {
        ...
        {
            RAnalyticsErrorLog(@"Failed to create table: %s", sqlite3_errmsg(result));
            return 0;
        }
