...
struct PushEventHandler {
    ...
    init(bundle: EnvironmentBundle, userDefaultsType: UserStorageHandleable.Type) {
        guard let appGroupId = bundle.object(forInfoDictionaryKey: RPushTrackingKeys.AppGroupIdentifierPlistKey) as? String else {
    ...
    func eventHasBeenSent(with trackingIdentifier: String?) -> Bool {
        guard let trackingIdentifier = trackingIdentifier,
              let domain = userStorageHandler?.dictionary(forKey: RPushTrackingKeys.OpenCountSentUserDefaultKey),
