...
{
    va_list argList;va_start (argList, format);va_end(argList);
    return [_RLogger log:RLoggingLevelVerbose format:format argList:argList];
}

+ (NSString * _Nullable )debug:(NSString *)format, ...
{
    va_list argList;va_start (argList, format);va_end(argList);
    return [_RLogger log:RLoggingLevelDebug format:format argList:argList];
}

+ (NSString * _Nullable )info:(NSString *)format, ...
{
    va_list argList;va_start (argList, format);va_end(argList);
    return [_RLogger log:RLoggingLevelInfo format:format argList:argList];
}

+ (NSString * _Nullable )warning:(NSString *)format, ...
{
    va_list argList;va_start (argList, format);va_end(argList);
    return [_RLogger log:RLoggingLevelWarning format:format argList:argList];
}

+ (NSString * _Nullable )error:(NSString *)format, ...
{
    va_list argList;va_start (argList, format);va_end(argList);
    return [_RLogger log:RLoggingLevelError format:format argList:argList];
}

+ (NSString * _Nullable)log:(RLoggingLevel)loggingLevel format:(NSString *)format argList:(va_list)argList
{
    if ([_RLogger loggingLevel] > loggingLevel || [_RLogger loggingLevel] == RLoggingLevelNone)
    {
        return nil;
    }
