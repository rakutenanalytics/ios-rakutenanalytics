...
struct AnalyticsManagerTests {
    ...
    struct TrackReferralAppTests {
        ...
        func testDoesNotTrackReferralAppWhenURLSchemeHasNoSourceApplication() {
            ...
            let didTrackEvent = analyticsManager.trackReferralApp(
                url: AnalyticsManagerTests.appURL,
                sourceApplication: nil
            )

            #expect(tracker.state == nil)
            #expect(!didTrackEvent)
        }
        ...
        func testDoesNotTrackReferralAppWhenUniversalLinkHasNoRef() {
            ...
            let didTrackEvent = analyticsManager.trackReferralApp(
                url: AnalyticsManagerTests.universalLinkURLWithoutRef,
                sourceApplication: nil
            )
        ...
        func testTracksReferralAppWhenURLSchemeIsOpened() {
            ...
            let didTrackEvent = analyticsManager.trackReferralApp(
                url: AnalyticsManagerTests.appURL,
                sourceApplication: AnalyticsManagerTests.bundleIdentifier
            )
        ...
        func testTracksReferralAppWhenUniversalLinkIsOpened() {
            ...
            let didTrackEvent = analyticsManager.trackReferralApp(
                url: AnalyticsManagerTests.universalLinkURL,
                sourceApplication: nil
            )
    ...
    @Suite("trackExternalReferralTests")
    struct TrackExternalReferralTests {
        var dependenciesContainer: SimpleContainerMock
        ...
        @Test("should track external referral when external source uses a Universal Link")
        func testTrackExternalReferralWhenUniversalLinkIsOpened() {
            let anayticsManager = AnalyticsManager(dependenciesContainer: dependenciesContainer)
            let tracker = TrackerMock()
            let url = AnalyticsManagerTests.universalLinkURLForExternalApp
            
            anayticsManager.add(tracker)
            anayticsManager.trackExternalReferral(url: url)
            
            #expect(tracker.state != nil)
            #expect(tracker.state?.referralTracking == ReferralTrackingType.externalReferral(url))
        }
    }
    
    @Suite("trackReferralsTests")
    struct TrackReferralsTests {
        var dependenciesContainer: SimpleContainerMock
        ...
        @Test("should track the referral app when a Universal Link is opened by a Rakuten app")
        func testTracksReferralAppWhenUniversalLinkIsOpenedByRakutenApp() {
            let analyticsManager = AnalyticsManager(dependenciesContainer: dependenciesContainer)
            let tracker = TrackerMock()

            analyticsManager.add(tracker)
            
            analyticsManager.trackReferrals(
                url: AnalyticsManagerTests.universalLinkURL,
                sourceApplication: nil
            )
        ...
        @Test("should track an external referral when a Universal Link is opened by an external app")
        func testTracksExternalReferralWhenUniversalLinkIsOpenedByExternalApp() {
            let anayticsManager = AnalyticsManager(dependenciesContainer: dependenciesContainer)
            let tracker = TrackerMock()
            let url = AnalyticsManagerTests.universalLinkURLForExternalApp
            
            anayticsManager.add(tracker)
            anayticsManager.trackReferrals(url: url)
    ...
    struct TryToTrackReferralAppWithSourceApplicationTests {
        ...
        @Test("should track an external referral when a URL Scheme is opened by an external app")
        func testTracksExternalReferralWhenURLSchemeIsOpenedByExternalApp() {
            let analyticsManager = AnalyticsManager(dependenciesContainer: dependenciesContainer)
            let tracker = TrackerMock()
            let url = AnalyticsManagerTests.universalLinkURLForExternalApp

            analyticsManager.add(tracker)
            analyticsManager.tryToTrackReferralApp(with: url, sourceApplication: nil)
