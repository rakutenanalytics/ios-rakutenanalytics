...
public class SwiftInterop: XCTestCase {
    ...
    public class TrackerSwift: NSObject, Tracker {
        ...
        public func process(event: AnalyticsManager.Event, state: AnalyticsManager.State) -> Bool {
            ...
            switch state.origin {
            case AnalyticsManager.State.Origin.inner, AnalyticsManager.State.Origin.external, AnalyticsManager.State.Origin.push: ()
            @unknown default: ()
            }
            
            switch state.loginMethod {
            case AnalyticsManager.State.LoginMethod.oneTapLogin, AnalyticsManager.State.LoginMethod.passwordInput, AnalyticsManager.State.LoginMethod.other: ()
            @unknown default: ()
            }
            ...
            case AnalyticsManager.Event.Name.initialLaunch:
                ...
                return true
            case AnalyticsManager.Event.Name.sessionStart, AnalyticsManager.Event.Name.sessionEnd, AnalyticsManager.Event.Name.install, AnalyticsManager.Event.Name.applicationUpdate: ()
