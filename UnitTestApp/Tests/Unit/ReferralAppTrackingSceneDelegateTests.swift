...
// which is only available in a running iOS application target.  SPM test targets run as plain
// executables without a UIKit application lifecycle, so connected scenes are never available there.
#if SWIFT_PACKAGE
#else
...
struct ReferralAppTrackingSceneDelegateTests {
    ...
    private static func ensureSuiteUIKitInitialized() {
        ...
        // `swizzleSceneDelegateFunctions` is idempotent (guarded by ObjectIdentifier set), but we
        // guard here too so the intent is explicit: swizzle once per suite, never again.
    ...
    struct WhenDelegateIsSetToNonNilValueTests {
        @Suite("When scene(_:willConnectTo:options:) is called")
        struct WhenSceneWillConnectToIsCalledTests {
            @Test("should process referral app tracking for a cold-launch URL scheme")
            @MainActor
            mutating func testShouldProcessReferralAppTrackingForColdLaunchURLScheme() async throws {
                ...
                #expect((spec.sceneDelegate as NSObject).responds(to: #selector(UISceneDelegate.scene(_:willConnectTo:options:))))
                spec.analyticsManager.handleIncomingColdLaunchFromInjectedValues()
            ...
            @Test("should process referral app tracking for a cold-launch universal link")
            @MainActor
            mutating func testShouldProcessReferralAppTrackingForColdLaunchUniversalLink() async throws {
                var spec = ReferralAppTrackingSceneDelegateTests()
                spec.setUp()
                defer { spec.tearDown() }
                ...
                UIScene.ConnectionOptions.DefaultValues.webpageURL = Payloads.universalLink
        ...
        @Suite("When the scene delegate is assigned twice")
        struct WhenSceneDelegateIsAssignedTwiceTests {
            @Test("should keep referral tracking active after a second delegate assignment")
            @MainActor
            mutating func testShouldKeepReferralTrackingActiveAfterSecondAssignment() async throws {
                var spec = ReferralAppTrackingSceneDelegateTests()
                spec.setUp()
                defer { spec.tearDown() }

                let scene = try #require(spec.windowScene)
                scene.delegate = spec.sceneDelegate
                scene.delegate = spec.sceneDelegate
